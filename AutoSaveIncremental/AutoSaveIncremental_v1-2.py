...
class AutoSaveIncrementalPreferencesPannel(bpy.types.AddonPreferences):
    """ """
    bl_idname = __name__

    time_btw_save = bpy.props.IntProperty(default=300) # seconds
    dir_path = bpy.props.StringProperty(subtype='DIR_PATH', default='//Incremental Save\\', description="Output directory for the render") # Voir chemin os + bouton setup adapté à l'os

    def draw(self, context):
        layout = self.layout
        layout.prop(self, "time_btw_save", text="Timer (seconds)")
        layout.prop(self, "dir_path", text="Auto-Save Directory")
        return {'FINISHED'}
...
def register():
    ...
    bpy.utils.register_class(AutoSaveIncrementalPreferencesPannel)
...
def unregister():
    ...
    bpy.utils.unregister_class(AutoSaveIncrementalPreferencesPannel)
